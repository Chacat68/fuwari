 ...
 * @param options 优化选项
 */
export function optimizeTitle(title: string, options: {
	maxLength?: number;
	addKeywords?: string[];
 ...
 * @param data 分享数据
 */
export function generateShareUrl(platform: 'twitter' | 'facebook' | 'linkedin' | 'weibo', data: {
	url: string;
	title: string;
	description?: string;
}): string {
...
export function calculateReadingTime(content: string, wordsPerMinute = 200): number {
	...
	const englishWords = cleanContent.replace(/[\u4e00-\u9fff]/g, '').split(/\s+/).filter(word => word.length > 0).length;
 ...
 */
export function containsSensitiveWords(content: string, sensitiveWords: string[] = []): boolean {
